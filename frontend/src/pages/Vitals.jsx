                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50 dark:bg-gray-900/50">
                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-widest">Biological Metric</th>
                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-widest">Precision Value</th>
                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-widest text-center">Reference Unit</th>
                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-widest">Trace Timestamp</th>
                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-widest text-right">Ops</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50 dark:divide-gray-900">
                      {history.slice(0, 5).map((h) => (
                        <tr key={h._id} className="group hover:bg-emerald-500/[0.04]">
                          <td className="px-8 py-6 text-sm font-bold text-gray-900 dark:text-white capitalize">{h.type.replace('_', ' ')}</td>
                          <td className="px-8 py-6 text-lg font-black text-emerald-500">{formatValue(h.type, h.value)}</td>
                          <td className="px-8 py-6 text-center text-xs font-black text-gray-400">{h.unit}</td>
                          <td className="px-8 py-6 text-xs font-bold text-gray-500">{new Date(h.timestamp).toLocaleString()}</td>
                    ...
                    <div key={lab._id} className="bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 p-8 rounded-[3rem] shadow-2xl space-y-6 group hover:border-emerald-500/20 transition-all">
                       ...
                       <div>
                          ...
                          <div className="flex items-baseline gap-2 mt-2">
                             <span className="text-2xl font-black text-emerald-500">{lab.resultValue}</span>
                          ...
                          <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-100 dark:border-white/5">
                             <div className="text-[8px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">Reference Alignment</div>
                             <div className="text-[11px] font-bold text-gray-600 dark:text-gray-300">{lab.referenceRange} {lab.unit}</div>
                          </div>
